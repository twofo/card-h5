<template name="yq-avatar">
	<view>
		...
		<canvas canvas-id="oper-canvas" id="oper-canvas" class="oper-canvas" :style="{top: sT, height: csH}" disable-scroll="false"
		 @touchstart="fStart" @touchmove="fMove" @touchend="fEnd"></canvas>
...
<script>
	...
	export default {
		...
		created() {
			...
			this.imgSrc.imgSrc = this.avatarSrc;
			this.letRotate = (this.canRotate === false || this.inner === true || this.inner === 'true' || this.canRotate ===
				'false') ? 0 : 1;
		...
		methods: {
							...
							success: r => {
								...
								if (!this.hasSel) {
									...
									if (this.selWidth && this.selHeight) {
										let sW = this.selWidth.toString().indexOf('upx') >= 0 ? parseInt(this.selWidth) * this.pxRatio :
											parseInt(
												this.selWidth),
											sH = this.selHeight.toString().indexOf('upx') >= 0 ? parseInt(this.selHeight) * this.pxRatio :
											parseInt(
												this.selHeight);
										style.width = sW + 'px';
										style.height = sH + 'px';
			...
			fUpload() {
				...
				let style = this.sS,
						...
						expHeight = this.eW || (height * this.pixelRatio);   //修改将this.eH改成 this.eW,输入图片要正方形
						
						console.log(this.eW,this.eH, width, height )
				// #ifdef MP-ALIPAY
				uni.showLoading();
				// #endif
				// #ifndef MP-ALIPAY
				...
				console.log(expWidth, expHeight)
			...
			fDrawImage(ini = false) {
				console.log("开始画图canvas")
				let tm_now = Date.now();
				console.log("当前时间",tm_now)
				console.log("上一次画图时间",this.drawTm)
				...
				let cc = this.cc,
					...
					console.log("画图大小",imgWidth, imgHeight)
				...
				console.log("this.init",this.isin)
				if (this.isin) {
					...
					cc.drawImage(this.imgPath, this.posWidth - this.centerX - cx, this.posHeight - this.centerY - cy, imgWidth,
						imgHeight);

				} else {
					console.log("画图的大小以及位置", -imgWidth / 2, -imgHeight / 2, imgWidth, imgHeight)
			...
			fMove(e) {
				...
				if (touch1) {
					...
					console.log("this.touch0",this.touch0)
