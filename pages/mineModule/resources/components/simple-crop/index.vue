<template>
	<view id="simple_crop" class="crop-whole-cover">
		<view class="crop-component" catchtouchend="endControl">
			...
			<view class="crop-rotate" :style="!rotateSlider?'display:none;':''">
				<scroll-view scroll-x="true" :scroll-left="curMoveX" catchscroll="scrollLineation">
					<view class="lineation" :style="'width:' + lineationWidth + 'px;'">
						<view v-for="(item, idx) in lineationArr" :key="idx">
			...
			<view class="crop-btns">
				...
				<view v-if="statusBtns.around" class="crop-around" @tap.stop="around"></view>
				<view v-if="statusBtns.reset" class="crop-reset" @tap.stop="reset"></view>
				<view v-if="statusBtns.crop" class="crop-btn" @tap.stop="getCropImage"></view>
			</view>
		</view>
	</view>
</template>
	...
	export default {
		...
		props: {
			...
			funcBtns: {
				// 功能按钮数组
				type: Array,
				default: () => ["close", "crop", "around", "reset"]
			},
			// borderDraw: {
			// 	// 裁剪框边框绘制函数
			// 	type: Function,
		...
		computed:{
			sigleObj(){
				const  { rotateSlider, startAngle, endAngle, gapAngle, lineationItemWidth } = this
				return { 
					rotateSlider, startAngle, endAngle, gapAngle, lineationItemWidth
				}
			},
			sizeObj(){
				const { size, cropSizePercent, positionOffset } = this
				return {
					size, cropSizePercent, positionOffset
				}
			},
			drawObj(){
				const { borderWidth, borderColor, boldCornerLen, coverColor } = this
				return {
					borderWidth, borderColor, boldCornerLen, coverColor
				}
			}
		},
		watch: {
			...
			drawObj: function() {
				if (this.isAttached) {
					var borderDraw = this.borderDraw;
					var coverDraw = this.coverDraw;
		...
		// 组件生命周期
		created: function() { // 在 created 生命周期中查看 this.data 数据时为默认值
		},
		...
		methods: {
			...
			transform: function(rotateCover, scaleKeepCover) {
				...
				transform += ' translateX(' + this._contentCurMoveX / scaleNum + 'px) translateY(' + this._contentCurMoveY /
					scaleNum + 'px)'; //移动
			...
			getCoverPointScale: function(point, rectPoints) {
				var pcv = this.getPCVectorProjOnUpAndRight(point, rectPoints); //计算矩形外一点到矩形中心向量在矩形边框向量上的投影距离

				var uLen = this.vecLen(pcv.uproj);
				var height = this.vecLen(pcv.up) / 2;
				var rLen = this.vecLen(pcv.rproj);
				var width = this.vecLen(pcv.right) / 2; //根据投影距离计算缩放倍数
			...
			getCoverTransform: function(transform, onlyTranslate) {
				...
				onlyTranslate = onlyTranslate ? onlyTranslate : false; //计算放大倍数
				...
				if (Math.abs(180 - uAng) < Math.abs(90 - uAng) || Math.abs(0 - uAng) < Math.abs(90 - uAng)) {
					//更接近180或者0
				...
				if (scale > 1) {
					...
					this.scaleTimes = this.scaleTimes * scale; //this._rotateScale = this._rotateScale * scale; // this._rotateScale 只能受旋转角度影响
					...
					if (outDetails.length > 0) {
						...
						if (moveX != 0 || moveY != 0) {
							for (i = 0; i < scalePoints.length; i++) {
								scalePoints[i].x = scalePoints[i].x + maxFarOut.iv.x, scalePoints[i].y = scalePoints[i].y + maxFarOut.iv.y;
							}
						}
					}
			...
			initRotateSlider: function() {
				...
				this._gapAngle = this.gapAngle;
				this._lineationItemWidth = this.lineationItemWidth;
				...
				console.log(self.$cropRotate)
				self.$cropRotate.boundingClientRect(function(rect) {
					console.log("裁剪---------width", rect)
					self.rotateWidth = rect.width;
					self._baseMoveX = -(lineationWidth * (0 - self._startAngle + self._gapAngle / 2) / (self._endAngle - self._startAngle +
						self._gapAngle) - self.rotateWidth / 2); //开始角度大于 0 且结束角度小于 0，以 0 度为起点

					var angle = self.rotateAngle - self._baseAngle;
					self._curMoveX = angle * lineationWidth / (self._endAngle - self._startAngle + self._gapAngle) + self._baseMoveX; //超出滚动边界
			...
			getRealCotentSize: function() {
				...
				this.contentHeight = this.originImage.height; //图片方向大于 4 时宽高互换
			...
			transformCoordinates: function(image) {
				...
				this.$cropFinal.width = this.size.width;
				this.$cropFinal.height = this.size.height;
				...
				imageCtx.restore(); //canvas 不受 css transform 影响 需要转换为 image 显示
			...
			getCropImage: function() {
				...
				var cropHeight = size.height; //需要考虑裁剪图片实际尺寸比裁剪尺寸小的情况
				...
				image1.onload = function() {
					...
					self.cropResultCtx.translate((self._contentCurMoveX + positionOffset.left) / scaleNum * ratio, (self._contentCurMoveY +
						positionOffset.top) / scaleNum * ratio);
					...
					wx.canvasToTempFilePath({
						...
						success(res) {
							...
							image2.onload = function() {
								...
								self.cropFinalCtx.drawImage(image2, (contentWidth - cropWidth) / 2, (contentHeight - cropHeight) / 2,
			...
			init: function() {
				...
				this.contentPoints = this.initContentPoints.slice(); //计算初始缩放倍数
			...
			isTwoFingerEvent: function(touches) {
				 ...
				 */
				if (this._isControl && this._downPoints && this._downPoints.length == 1 && touches.length >= 2 && (touches[0].clientX ==
						this._downPoints[0].clientX && touches[0].clientY == this._downPoints[0].clientY || touches[1].clientX == this._downPoints[
							0].clientX && touches[1].clientY == this._downPoints[0].clientY)) {
					return true;
				}
			...
			endControl: function() {
				if (this._isControl) {
					...
					if (!this.isWholeCover(this.contentPoints, this.cropPoints)) {
						//如果没有完全包含则需要进行适配变换
						var scaleNum = this.scaleTimes / this.times * this._rotateScale;
						var transform = '';
						transform += ' scale(' + scaleNum + ')'; //缩放
						...
						transform += ' rotate(' + this.rotateAngle + 'deg) '; //适配变换
			...
			updateBox: function() {
				...
				var positionOffset = this.positionOffset;
				this.times = size.width / this.maskViewSize.width > size.height / this.maskViewSize.height ? size.width / this.maskViewSize
					.width / cropSizePercent : size.height / this.maskViewSize.height / cropSizePercent;
				this.cropRect = {
			...
			defaultBorderDraw: function($cropCover) {
				...
				this.cropCoverContext.fillRect(0, 0, coverWidth, coverHeight); //绘制边框（边框内嵌）
				...
				if (boldCornerLen > 0) {
					//边框四个角加粗
					this.cropCoverContext.fillRect(borderRect.left - borderWidth, borderRect.top - borderWidth, boldCornerLen,
						boldCornerLen); //左上角

					this.cropCoverContext.fillRect(borderRect.left + borderRect.width - boldCornerLen + borderWidth, borderRect.top -
						borderWidth, boldCornerLen, boldCornerLen); //右上角

					this.cropCoverContext.fillRect(borderRect.left - borderWidth, borderRect.top + borderRect.height - boldCornerLen +
						borderWidth, boldCornerLen, boldCornerLen); //左下角

					this.cropCoverContext.fillRect(borderRect.left + borderRect.width - boldCornerLen + borderWidth, borderRect.top +
						borderRect.height - boldCornerLen + borderWidth, boldCornerLen, boldCornerLen); //右下角
				} //清空内容区域


				this.cropCoverContext.clearRect(borderRect.left + borderWidth, borderRect.top + borderWidth, borderRect.width - 2 *
					borderWidth, borderRect.height - 2 * borderWidth);
			},
			// 矩形位置形式转换为顶点坐标形式
			...
			touchstart: function(event) {
				...
				if (this._downPoints && this._downPoints.length >= 2) {
					...
					this.fingerLen = Math.sqrt(Math.pow(this._downPoints[0].clientX - this._downPoints[1].clientX, 2) + Math.pow(this
						._downPoints[0].clientY - this._downPoints[1].clientY, 2));
					this.fingerScale = 1;
					this.fingerCenter = {
						//双指操作中心
			...
			touchmove: function(event) {
				if (this._downPoints && this._downPoints.length > 0) {
					if (!this._multiPoint) {
						...
						var newFingerLen = Math.sqrt(Math.pow(touches[0].clientX - touches[1].clientX, 2) + Math.pow(touches[0].clientY -
							touches[1].clientY, 2));
