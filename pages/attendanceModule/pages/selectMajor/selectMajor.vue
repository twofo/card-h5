...
export default {
  ...
  onLoad: function (options) {
    console.log('------------------------选择组织------------------------');
    console.log('院系跳转到专业的数据', options.parentId);
  ...
  methods: {
    ...
    getAttendOrg: function (userInfoCach, id) {
      ...
      wx.request({
        url: app.globalData.RequestUrl.attendanceRequestUrl.getAttendOrg + '?type=2&teacherId=' + userInfoCach.personId + '&id=' + id,
        method: 'GET',
        header: {
        ...
        success: function (res) {
          ...
          if (res.data && res.data.code == 200) {
            for (let i = 0; i < res.data.rows.length; ++i) {
              ...
              const departmentCach = app.globalData.StorageUtil.get('majorCach');
    ...
    onMyEvent: function (e) {
      ...
      if (checkboxItems) {
        ...
        app.globalData.StorageUtil.set('majorCach', checkboxItems);
      } // valueType=1 组件点击下级 valueType=2 組件点击确定 


      if (valueType == 1) {
        app.globalData.PublicUtil.navigateByUrl(app.globalData.PageUrl.attendancePageUrl.selectClasslUrl + '?parentId=' + parentId, 500);
      } else if (valueType == 2) {
        wx.navigateBack({
          delta: 2
