...
export default {
  ...
  onLoad: function (options) {
    ...
    if (options.type == 1) {
      ...
      this.getAttendOrg(userInfoCach, options.parentId);
    }
  },
  ...
  methods: {
     ...
     * 获取考勤组织
     */
    getAttendOrg: function (userInfoCach, id) {
      ...
      wx.request({
        url: app.globalData.RequestUrl.attendanceRequestUrl.getAttendOrg + '?type=3&teacherId=' + userInfoCach.personId + '&id=' + id,
        method: 'GET',
        header: {
        ...
        success: function (res) {
          ...
          if (res.data && res.data.code == 200) {
            for (let i = 0; i < res.data.rows.length; ++i) {
              ...
              const departmentCach = app.globalData.StorageUtil.get('classCach');
    ...
    selectPersonnel: function (userInfoCach, id) {
      ...
      wx.request({
        url: app.globalData.RequestUrl.appointmentRequestUrl.selectPersonnel + '?selectType=3&parentId=' + id,
        method: 'GET',
        header: {
          'token': userInfoCach.token
        },
    ...
    onMyEvent: function (e) {
      ...
      if (checkboxItems) {
        console.log('选中的班级----', checkboxItems);
        app.globalData.StorageUtil.set('classCach', checkboxItems);
      } // valueType=1 组件点击下级 valueType=2 組件点击确定 


      if (valueType == 1) {
        app.globalData.PublicUtil.navigateByUrl(app.globalData.PageUrl.attendancePageUrl.selectStudentUrl + '?parentId=' + parentId, 500);
      } else if (valueType == 2) {
        wx.navigateBack({
          delta: 3
